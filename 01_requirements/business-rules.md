...
BN-016 — Semana de 5 días, Ahorro+Meta combinados, Corte semanal, panel responsivo
Descripción: Ronda final de ajustes sobre el Dashboard y el panel en general.
Semana laboral de 5 días (no 7): `weeklyEquivalent()` y la división final de `computeOperationalDailyNeed` ahora usan `WORK_DAYS_PER_WEEK = 5` en vez de 7 — Renta/Gastos fijos con periodicidad DIARIO se multiplican ×5, y el total semanal se reparte entre 5 días. Nómina ya usaba 5 (sin cambios). La ventana de "Gastos del día" también pasó de 7 a 5 días.
Renta con valor diario visible: la pestaña Renta (en "Renta, Nómina y Gastos") ahora muestra, junto al valor activo, su equivalente diario (misma fórmula de 5 días) — sin necesidad de ir al Dashboard.
Ahorro operativo + Meta de ganancia en un solo cartón, con un total combinado nuevo: `ahorroDiarioNecesario (gastos) + gananciaDiariaNecesaria (meta)` = cuánto generar en ventas por día para cubrir gastos y avanzar hacia la meta. La Meta se puede editar sin salir del Dashboard (botón "Editar meta" → modal que actualiza los `Setting` `finance.profitGoal*` directamente).
"Corte semanal": nuevo tablero (`computeWeeklySummary` + `GET /api/reports/weekly-summary`) con Ventas/Gastos/Ganancia de las últimas 8 semanas.
Panel responsivo: `AdminLayout` — el sidebar fijo de 256px rompía la vista en celular ("se veía gigante"). Ahora es un cajón (drawer) que se abre con un botón ☰ en pantallas chicas y queda fijo en pantallas medianas/grandes (Tailwind `md:`).
Errores de imagen más claros: `ImageUploadInput` ya no bloquea por tipo de archivo de antemano — intenta cargar cualquier imagen, y si el navegador no puede decodificarla (típicamente HEIC de iPhone), muestra un mensaje específico explicando por qué y qué hacer.
Impacto: `financeCalculations.ts` (fórmula + nueva función), `reports.ts` (nuevo endpoint), `DashboardPage.tsx` (cartón combinado + corte semanal + edición de meta), `GestionOperativaPage.tsx` (valor diario de renta), `AdminLayout.tsx` (responsivo), `ImageUploadInput.tsx` (errores). Nada de lo existente se quitó.
